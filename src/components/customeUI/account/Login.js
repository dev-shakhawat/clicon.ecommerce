...
export default function Login() {
  ...
  return (
    <div className='p-8 bg-white lg:min-w-[360px]'>
      ...
      {/* title */}
      <h2 className=" font-Public_Sans font-semibold text-xl text-center text-[#191C1F]">Sign in to your account</h2>

      {/* login form */}
      <form onSubmit={handleLogin}>
        {/* Email */}
        <div className="mt-5">
          <label htmlFor="email" className="font-Public_Sans text-sm">Email Address</label>
          <input
            type="email"
            id="email"
            value={email}
        ...
        <div className="mt-4 relative">
          <div className="flex justify-between text-sm">
            <span>Password</span>
            <button type="button" onClick={() => dispatch(ActiveModalSet('forgetPass'))} className="text-[#2DA5F3]">
              Forgot Password?
            </button>
          </div>
          <input
